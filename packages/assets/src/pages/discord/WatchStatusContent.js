...
import {
  Card, IndexTable, Text, Badge, Button, InlineStack, BlockStack, Banner, Spinner
...
export default function WatchStatusContent({watchStatuses, onRefresh}) {
  ...
  // Fetch Gmail accounts to show all (not just watched ones)
  ...
  const handleStartWatch = async (email) => {
    try {
      ...
      const res = await api('/api/gmail/watch', {
        method: 'POST',
  ...
  const handleStopWatch = async (email) => {
    try {
      setActionLoading(email);
      await api('/api/gmail/watch/stop', {
        method: 'POST',
        body: JSON.stringify({email})
      });
      onRefresh();
  ...
  const getStatusBadge = (watch) => {
    ...
    if (watch.status === 'active') {
      const hoursLeft = Math.round((new Date(watch.watchExpiration) - Date.now()) / (1000 * 60 * 60));
  ...
  if (accounts.length === 0) {
    return (
      <Card>
        <Banner tone="info">
          No Gmail accounts connected. Go to <strong>Email Management → Accounts</strong> to connect one first.
        </Banner>
      </Card>
    );
  }
  ...
  const rowMarkup = accounts.map((account, index) => {
    ...
    return (
      <IndexTable.Row id={account.email} key={account.email} position={index}>
        <IndexTable.Cell><Text fontWeight="bold">{account.email}</Text></IndexTable.Cell>
        ...
        <IndexTable.Cell>
          {watch?.renewalError
            ? <Text as="span" tone="critical" variant="bodySm">{watch.renewalError}</Text>
            : <Text as="span" tone="subdued">-</Text>
          }
        </IndexTable.Cell>
        <IndexTable.Cell>
          {isActive ? (
            <Button onClick={() => handleStopWatch(account.email)} size="slim" tone="critical"
              loading={actionLoading === account.email} disabled={!!actionLoading}>
              Stop
            </Button>
          ) : (
            <Button onClick={() => handleStartWatch(account.email)} size="slim" variant="primary"
              loading={actionLoading === account.email} disabled={!!actionLoading}>
              Start Watch
            </Button>
          )}
    ...
    <Card>
      <BlockStack gap="400">
        <InlineStack align="space-between">
          <Text as="h2" variant="headingMd">Gmail Watch Status</Text>
          <Button onClick={() => { onRefresh(); fetchAccounts(); }} size="slim">Refresh</Button>
        </InlineStack>

        <Banner tone="info">
          Click <strong>"Start Watch"</strong> to begin listening for new emails on an account.
          New emails will be auto-forwarded to Discord based on your filter rules.
        </Banner>
        ...
        <IndexTable
          ...
          headings={[
            {title: 'Email'}, {title: 'Status'}, {title: 'Expires'},
            {title: 'Last Sync'}, {title: 'Error'}, {title: 'Actions'}
          ]}
          selectable={false}
        >
          {rowMarkup}
