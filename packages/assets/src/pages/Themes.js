...
export default function Themes() {
  ...
  const loadThemes = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(`Failed to load themes for "${storeName}".`);
    } finally {
  ...
  const loadImportedThemes = async () => {
    try {
      ...
      setErrorMsg('Failed to load saved themes');
    } finally {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (ok > 0) {
        setSuccessMsg(
          `Theme imported to ${ok} store(s)${
            results.length - ok > 0 ? `, ${results.length - ok} failed` : ''
          }.`
        );
        setThemeName('');
        setThemeFile(null);
  ...
  const tabs = [
    ...
    {
      id: 'library',
      content: `Theme Library${importedThemes.length > 0 ? ` (${importedThemes.length})` : ''}`,
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        title="Import Theme"
        primaryAction={{
          content: `Import to ${selectedImportStores.length || 0} Store${
