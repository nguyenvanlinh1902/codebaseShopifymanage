...
export async function processPushNotification(messageData) {
  ...
  // Update historyId — Discord forwarding is handled by discordDigestCron
  await watchRepo.updateHistoryId(emailAddress, newHistoryId);

  console.log(`${LOG_PREFIX} Processed ${newMessages.length} new messages for ${emailAddress} (digest cron will forward)`);
  return {emailAddress, storeId: watchRecord.storeId, messagesCount: newMessages.length};
}
